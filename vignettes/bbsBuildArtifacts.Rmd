...
in the package at `demo_url()`.  If `url` is not supplied
...
```{r lkhd}
package_by_host_data(af@pkg_paths["affyPara"], host="tokay2")
```
...
```{r domore}
few = allpaths[1:10]
dcfs = lapply(few, function(x) simplify_artifact_build_dcfs(package_by_host_data(x)$dcfs))
